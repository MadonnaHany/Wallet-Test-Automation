...
double balanceamountbefore_loadwallet = CustomKeywords.'db_operations.MyWalletDBOperations.GetBalance'(GlobalVariable.MobileNumber)
...
WebUI.callTestCase(findTestCase('Shared_Testcases/try to make transaction with valid M-Pin'), [('TransactionStatus') : transaction_status
        , ('ExpectedMessage') : expected_message])

double balanceamountafter_loadwallet

if ((expected_message == 'Successful Transaction') || (expected_message == 'Successful transaction')) {
    balanceamountafter_loadwallet = CustomKeywords.'db_operations.MyWalletDBOperations.GetBalance'(GlobalVariable.MobileNumber)

    Mobile.delay(40, FailureHandling.CONTINUE_ON_FAILURE)

    Mobile.verifyGreaterThan(balanceamountafter_loadwallet, balanceamountbefore_loadwallet)
} else {
    balanceamountafter_loadwallet = CustomKeywords.'db_operations.MyWalletDBOperations.GetBalance'(GlobalVariable.MobileNumber)
